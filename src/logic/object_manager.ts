...
class Object_manager{
    ...
    constructor(scene: Phaser.Scene){
        ...
        this.ignoreObjectLabels = []; // Objects that are not in this.objects only!
        $("#content").append
(`
...
`)
    ...
    collisionHandler(event: Phaser.Physics.Matter.Events.CollisionStartEvent){
        for (var i = 0; i < event.pairs.length; i++) {
            ...
            if(this.ignoreObjectLabels.includes(objectA)){pairs_found++}
            var objectB = bodyB.label;
            if(this.ignoreObjectLabels.includes(objectB)){pairs_found++}
            var pairs_found = 0;
            if(pairs_found < 2){
    ...
    fireProjectileArrow(angle: number, speed: number, owner: any, dmg: number, ignore: string[], lifespan: number){
    ...
    summonGoblin(hp: number, speed: number, x: number, y: number, reward: number){
    ...
    summonBoss(hp: number, speed: number, x: number, y: number, reward: number){
    ...
    getEntityBySprite(sprite: Phaser.Physics.Matter.Sprite){
        for (let index = 0; index < this.objects.length; index++) {
            const element = this.objects[index];
...
/*(`
<div id="hp" style="pointer-events: none;"><div class="fill"></div></div>
<div id="highscore" style="pointer-events: none;"></div>
<div id="score" style="pointer-events: none;"></div>
`)*/
